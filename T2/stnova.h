...
typedef struct 
{
    char user[TAMANHO_CHAVE];
    char alvo[TAMANHO_CHAVE];
    char *texto; 
} st_mensagem;

typedef struct st_elem
{
    int prioridade; //num de msg a->b + b->a
    st_mensagem* msg; // apontador das mensagens a->b
    struct st_elem * proximo;
} st_elemento;

typedef struct
{
 	int st_tamanho; // num de instancias (num de pessoas que envia msg) 
	st_elemento** st_elementos; //vetor de apontadores para prioridades 
} estrutura;
